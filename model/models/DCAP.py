...
class DCAP(torch.nn.Module):
    def __init__(self, field_dims, embed_dim, num_heads=2, num_layers=3, mlp_dims=(400, 400, 400), dropouts=(0.5, 0.5)):
        super().__init__()
...
class MultiheadAttentionInnerProduct(torch.nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, need_weights=False):
        ...
        q = q.view(-1, bsz * self.num_heads, self.head_dim).transpose(0,
                                                                      1)  # [batch size * num_heads, num_fields, head_dim]
        q = q * self.scale
        k = self.linear_k(key)
