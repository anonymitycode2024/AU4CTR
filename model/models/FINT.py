...
class FINT_CL(StarCL):
    def __init__(self, field_dims, embed_dim, mlp_layers=(400, 400, 400), num_deep=3, dropout=0.5,pratio=0.5):
        super(FINT_CL, self).__init__(field_dims, embed_dim, pratio)
        ...
        self.projection = nn.Linear(self.embed_output_size, embed_dim)

    def feature_interaction(self, x_emb):
        x_fint = x_emb
        for i in range(self.num_deep):
            x_fint = x_fint * torch.matmul(self.fints_param[i], x_emb) + self.Ul[i] * x_fint
        return x_fint.view(x_fint.size(0), -1)

    def forward(self, x):
        x_emb = self.embedding(x)
        fi_out = self.feature_interaction(x_emb)
        pred_y = self.mlp(fi_out)
        return pred_y